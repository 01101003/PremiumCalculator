...
methods: {
  initGeoGebra() {
    // Ensure GGBApplet is available
    if (!window.GGBApplet) {
      console.error('GeoGebra applet not loaded');
      return;
    }

    // Check if the element exists
    if (!this.$refs.ggbElement) {
      console.error('GeoGebra container not found');
      return;
    }

    // Configuration for GeoGebra applet
    const params = {
      "appName": "geometry",
    ...
    this.ggbApp = new window.GGBApplet(params, true);
    this.ggbApp.inject(this.$refs.ggbElement);
  },
  handleResize() {
    if (this.ggbApp && this.$refs.ggbElement) {
      const newWidth = window.innerWidth - 340;
      const newHeight = window.innerHeight - 40;
      this.ggbApp.setSize(newWidth, newHeight);
    }
  },
  updateMeasurements(api, objName) {
    ...
    switch(objectType) {
      ...
      case 'polygon':
        ...
        measurementsHTML += `Area: ${area.toFixed(2)} sq units<br>`;
        measurementsHTML += `Perimeter: ${perimeter.toFixed(2)} units<br>`;
      ...
      case 'circle':
        ...
        measurementsHTML += `Area: ${circleArea.toFixed(2)} sq units<br>`;
        break;
