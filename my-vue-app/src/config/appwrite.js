...
module.exports = async function(req, res) {
  ...
  const { query, operation, format } = JSON.parse(req.payload || '{}');
  
  // Validate input
  if (!query) {
    return res.json({ error: 'Missing query parameter' }, 400);
  }
  
  try {
    // Wolfram API key should be set as an environment variable in Appwrite
    const appId = process.env.WOLFRAM_APP_ID;
    if (!appId) {
      throw new Error('Wolfram API key is not configured');
    }
    
    // Construct the Wolfram API URL
    const wolframUrl = `https://api.wolframalpha.com/v2/query?input=${encodeURIComponent(query)}&appid=${appId}&podstate=Step-by-step%20solution&format=${format || 'plaintext'}`;
    
    // Call Wolfram API
    const response = await axios.get(wolframUrl);
    
    // Process Wolfram response
    let result = '';
    let steps = '';
    let plot = '';
    
    // Parse XML response from Wolfram
    // This is simplified - you'd need proper XML parsing
    if (response.data) {
      // Extract the primary result
      const resultMatch = /<pod title='Result'.*?>(.*?)<\/pod>/s.exec(response.data);
      if (resultMatch) {
        result = resultMatch[1].replace(/<plaintext>(.*?)<\/plaintext>/s, '$1');
      }
      
      // Extract step-by-step solution
      const stepsMatch = /<pod title='Step-by-step solution'.*?>(.*?)<\/pod>/s.exec(response.data);
      if (stepsMatch) {
        steps = stepsMatch[1].replace(/<plaintext>(.*?)<\/plaintext>/s, '$1');
      }
      
      // Extract plot if available
      const plotMatch = /<pod title='Plot'.*?>(.*?)<\/pod>/s.exec(response.data);
      if (plotMatch && plotMatch[1].includes('<img src=')) {
        const imgSrcMatch = /<img src='(.*?)'/s.exec(plotMatch[1]);
        if (imgSrcMatch) {
          plot = `<img src="${imgSrcMatch[1]}" alt="Plot">`;
        }
      }
    }
    ...
    return res.json({ error: error.message || 'Error processing request' }, 500);
  }
};